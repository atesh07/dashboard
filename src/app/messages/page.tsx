...
import { MessageSquare, Users, Shield, Bell, ArrowRight, CheckCircle2, Zap } from 'lucide-react';
...
export default function MessagesComingSoon() {
  ...
  const upcomingFeatures = [
    ...
    { icon: Users, text: "Team Channels" },
    { icon: Zap, text: "Instant File Sharing" },
  ];
  ...
  return (
    <div className="min-h-[85vh] flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden text-gray-900 dark:text-slate-300 transition-colors duration-300">
      ...
      <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-blue-500/20 dark:bg-blue-600/10 rounded-full blur-[80px] animate-pulse"></div>
      <div className="absolute bottom-1/4 left-1/4 w-72 h-72 bg-indigo-500/20 dark:bg-indigo-600/10 rounded-full blur-[80px] animate-pulse delay-700"></div>
      ...
      <div className={`max-w-2xl w-full z-10 transition-all duration-1000 ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] text-center relative overflow-hidden">
          ...
          {/* Animated Chat Icon */}
          <div className="mx-auto w-24 h-24 bg-gradient-to-tr from-blue-100 to-indigo-50 dark:from-slate-800 dark:to-slate-800 border border-blue-200 dark:border-slate-700 rounded-[2rem] flex items-center justify-center mb-8 shadow-lg transition-transform hover:scale-110 duration-300 relative group">
            <div className="absolute inset-0 bg-blue-500/20 dark:bg-blue-500/10 rounded-[2rem] blur-xl group-hover:bg-blue-500/30 transition-colors"></div>
            {/* The chat bubble floats up and down smoothly */}
            <MessageSquare size={40} className="text-blue-600 dark:text-blue-400 relative z-10 animate-[bounce_3s_infinite]" />
            {/* Notification dot that pulses */}
            <div className="absolute top-6 right-6 w-3 h-3 bg-rose-500 rounded-full border-2 border-white dark:border-slate-800 animate-pulse z-20"></div>
          </div>
          ...
          <p className="text-gray-500 dark:text-slate-400 max-w-md mx-auto mb-10 leading-relaxed text-sm md:text-base">
            We are building a seamless, secure way to communicate with your clients, advisors, and team members directly within Zorvyn.
          </p>
            ...
            {isSuccess ? (
              ...
              <form onSubmit={handleNotify} className="flex items-center relative">
                ...
                <button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-70 flex items-center gap-2 group shrink-0"
                >
